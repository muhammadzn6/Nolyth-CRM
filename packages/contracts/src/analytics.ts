...
export const analyticsKpisSchema = z.strictObject({ applications: z.number().int().nonnegative(), responses: z.number().int().nonnegative(), interviews: z.number().int().nonnegative(), offers: z.number().int().nonnegative(), acceptedOffers: z.number().int().nonnegative(), placements: z.number().int().nonnegative(), starts: z.number().int().nonnegative(), activePipeline: z.number().int().nonnegative(), overdueTasks: z.number().int().nonnegative(), responseRate: z.number().nonnegative().nullable(), averageResponseTimeHours: z.number().nonnegative().nullable().optional() });
