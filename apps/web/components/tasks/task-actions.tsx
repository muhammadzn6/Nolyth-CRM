...
export function TaskActions({ taskId, version }: { taskId: string; version: number }) {
  ...
  return <div className="mt-4 grid gap-2">
    <div className="flex flex-wrap gap-2"><Button disabled={pending} onClick={() => void finish()} size="sm">{pending ? "Saving…" : "Complete"}</Button><Button disabled={pending} onClick={() => setShowCancel((value) => !value)} size="sm" variant="ghost">Cancel</Button></div>
    {showCancel ? <div className="flex flex-col gap-2 sm:flex-row"><label className="sr-only" htmlFor={`cancel-${taskId}`}>Cancellation reason</label><Input id={`cancel-${taskId}`} onChange={(event) => setReason(event.target.value)} placeholder="Cancellation reason" value={reason} /><Button disabled={pending || !reason.trim()} onClick={() => void cancel()} size="sm" variant="secondary">Confirm cancellation</Button></div> : null}
