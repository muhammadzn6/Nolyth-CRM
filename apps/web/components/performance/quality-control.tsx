...
export function QualityControl({ quality }: { quality: PerformanceQualityIndicators }) {
  ...
  return <section aria-label="Application quality guardrails" className="rounded-[26px] border border-border bg-card p-5 shadow-sm sm:p-6">
    ...
    <ul className="mt-4 grid gap-2 sm:grid-cols-2">{indicators.map(([icon, label, value]) => <li className="flex min-h-16 items-center justify-between gap-3 rounded-2xl bg-surface-subtle px-3 py-2.5" key={label}><span className="inline-flex min-w-0 items-center gap-2 text-xs text-muted-foreground"><span aria-hidden="true" className="grid size-6 shrink-0 place-items-center rounded-full bg-card font-bold text-primary">{icon}</span><span title={label} className="truncate">{label}</span></span><strong className="text-sm text-foreground">{rate(value)}</strong></li>)}</ul>
  </section>;
}
