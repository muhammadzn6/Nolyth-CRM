  ...
  return <section aria-label="BD performance leaderboard" className="rounded-[26px] border border-border bg-card p-5 shadow-sm sm:p-6">
    ...
    <ol className="mt-5 space-y-3">
      ...
      {rows.map((row) => <li className="rounded-2xl border border-border bg-background/70 p-4" key={row.bdId}>
        ...
        <div className="mt-3 grid gap-2 text-xs text-muted-foreground sm:grid-cols-3">{[[metricIcons[0], `${percent(row.performance.effectiveTargetAttainmentPercent)} attainment`], [metricIcons[1], `${percent(row.performance.followUpSlaCompliancePercent)} follow-up SLA`], [metricIcons[2], `${percent(row.performance.maturedOutcomeScorePercent)} outcomes`]].map(([icon, label]) => <span className="inline-flex items-center gap-1.5" key={label}><span aria-hidden="true" className="text-primary">{icon}</span><span title={label}>{label}</span></span>)}</div>
        <div className="mt-3 flex flex-wrap items-center gap-2">{row.warnings.map((warning) => <span className="inline-flex items-center gap-1 rounded-full bg-warning-soft px-2 py-1 text-[10px] font-semibold text-warning-foreground" key={warning} title={warningLabel(warning)}><span aria-hidden="true">!</span>{warningLabel(warning)}</span>)}<span className="rounded-full bg-surface-subtle px-2 py-1 text-[10px] font-semibold text-muted-foreground" title="Score coverage">{row.performance.scoreCoverage.replaceAll("_", " ")}</span><a aria-label={`Open score details for ${row.bdName}`} className="ml-auto text-lg font-bold leading-none text-primary" href={`/?performancePeriod=${period}&performanceMetric=QUALIFIED_APPLICATIONS&performanceBdId=${row.bdId}`} title="Score details">→</a></div>
      </li>)}
    </ol>
