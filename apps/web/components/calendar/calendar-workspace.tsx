...
import { calendarDateForTimeZone } from "../../lib/business-day";
...
type CalendarSelection = { kind: CalendarItem["kind"]; id: string };
type SlotDateTimeValues = { startsAt: string; endsAt: string; startsAtIso: string; endsAtIso: string };
type SelectedSlot = { startsAtIso: string; initialValues?: SlotDateTimeValues };
...
export function slotDateTimeValue(date: Date, hour: number): string {
  return `${dateKey(date)}T${String(hour).padStart(2, "0")}:00`;
}

function slotDateTimeValues(startsAtIso: string, targetTimezone: string): SlotDateTimeValues {
  const endsAtIso = new Date(new Date(startsAtIso).getTime() + 60 * 60 * 1000).toISOString();
  return {
    startsAt: isoToZonedLocalDateTime(startsAtIso, targetTimezone),
    endsAt: isoToZonedLocalDateTime(endsAtIso, targetTimezone),
    startsAtIso,
    endsAtIso,
  };
}
...
export function CalendarWorkspace({ actor, interviews, externalMeetings = [], embedded = false, leads = [], closers = [], leadTimezones = {}, initialDate, businessTimeZone = primaryTimezone }: { actor: SessionUser; interviews: InterviewSummary[]; externalMeetings?: CloserDashboardExternalMeeting[]; embedded?: boolean; leads?: LeadSummary[]; closers?: UserSummary[]; leadTimezones?: Record<string, string>; initialDate?: string; businessTimeZone?: string }) {
  const router = useRouter();
  const [view, setView] = useState<View>("day");
  const [anchor, setAnchor] = useState(() => {
    const selected = initialDate ? new Date(initialDate) : null;
    return calendarDateForTimeZone(selected && !Number.isNaN(selected.getTime()) ? selected : new Date(), businessTimeZone);
  });
  ...
  const [detailsSelection, setDetailsSelection] = useState<CalendarSelection | null>(null);
  const [selectedSlot, setSelectedSlot] = useState<SelectedSlot | null>(null);
  const [rescheduling, setRescheduling] = useState(false);
  const [calendarError, setCalendarError] = useState<string>();
  const [enabledTimezones, setEnabledTimezones] = useState<string[]>([businessTimeZone, secondaryTimezone]);
  ...
  useEffect(() => {
    ...
    const { hour, minute } = timeParts(now.toISOString(), businessTimeZone);
  ...
  const selectedDetails = detailsSelection
    ? items.find((item) => item.kind === detailsSelection.kind && item.value.id === detailsSelection.id) ?? null
    : null;
  const selectDetails = (item: CalendarItem) => setDetailsSelection({ kind: item.kind, id: item.value.id });
  useEffect(() => {
    if (detailsSelection && !selectedDetails) setDetailsSelection(null);
  }, [detailsSelection, selectedDetails]);
  const dates = datesForView(anchor, view);
  const grouped = useMemo(() => new Map(dates.map((date) => [dateKey(date), items.filter((item) => dateKeyInTimezone(new Date(itemDates(item).startsAt), businessTimeZone) === dateKey(date))])), [businessTimeZone, dates, items]);
  ...
  const handleSelectSlot = (slot: { date: Date; hour: number }) => {
    try {
      const startsAtIso = zonedLocalDateTimeToIso(slotDateTimeValue(slot.date, slot.hour), businessTimeZone);
      const initialTimezone = leadTimezones[leads[0]?.id ?? ""];
      const initialValues = initialTimezone ? slotDateTimeValues(startsAtIso, initialTimezone) : undefined;
      setCalendarError(undefined);
      setSelectedSlot({ startsAtIso, initialValues });
    } catch (cause) {
      setSelectedSlot(null);
      setCalendarError(cause instanceof Error ? cause.message : "Interview time could not be prepared.");
    }
  };
  const handleReschedule = async (item: CalendarItem, date: Date, hour: number) => {
    ...
    setCalendarError(undefined);
    try {
      const startsAtIso = zonedLocalDateTimeToIso(`${dateKey(date)}T${String(hour).padStart(2, "0")}:00`, businessTimeZone);
      ...
      const originalStartsAt = isoToZonedLocalDateTime(startsAtIso, item.value.timezone);
      const originalEndsAt = isoToZonedLocalDateTime(endsAtIso, item.value.timezone);
      await updateInterview(item.value.id, { startsAt: startsAtIso, endsAt: endsAtIso, timezone: item.value.timezone, originalDatetimeText: `${originalStartsAt}–${originalEndsAt} (${item.value.timezone})`, expectedVersion: item.value.version });
      router.refresh();
    } catch (cause) {
      setCalendarError(cause instanceof Error ? cause.message : "Interview could not be rescheduled.");
...
function CalendarNavigation({ move, onToday }: { move: (amount: number) => void; onToday: () => void }) {
  return <div className="flex flex-wrap gap-2"><button className="rounded-lg border border-border bg-surface px-3 py-2 text-sm font-semibold text-foreground hover:bg-surface-subtle" onClick={onToday} type="button">Today</button><div className="flex rounded-lg border border-border bg-surface p-1"><button aria-label="Previous period" className="size-8 rounded-md text-lg text-muted-foreground hover:bg-surface-subtle" onClick={() => move(-1)} type="button">‹</button><button aria-label="Next period" className="size-8 rounded-md text-lg text-muted-foreground hover:bg-surface-subtle" onClick={() => move(1)} type="button">›</button></div></div>;
}
...
function MonthGrid({ anchor, businessTimeZone, dates, grouped, onSelect }: { anchor: Date; businessTimeZone: string; dates: Date[]; grouped: Map<string, CalendarItem[]>; onSelect: (item: CalendarItem) => void }) {
  return <div className="min-w-[720px]"><div className="grid grid-cols-7 border-b border-border bg-surface-subtle">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <p className="border-r border-border px-2 py-2.5 text-center text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground last:border-r-0" key={day}>{day}</p>)}</div><div className="grid grid-cols-7">{dates.map((date) => { const items = grouped.get(dateKey(date)) ?? []; const outside = date.getMonth() !== anchor.getMonth(); return <div className={`min-h-[96px] border-b border-r border-border p-2 ${outside ? "calendar-month-inactive-cell bg-background/50" : "calendar-month-active-cell bg-surface"}`} key={dateKey(date)} data-testid={outside ? "calendar-inactive-month-date" : "calendar-active-month-date"}><p className={`text-xs font-semibold ${dateKey(date) === dateKey(calendarDateForTimeZone(new Date(), businessTimeZone)) ? "grid size-6 place-items-center rounded-full bg-primary text-primary-foreground" : outside ? "text-muted-foreground/50" : "text-muted-foreground"}`}>{formatMonthDate(date)}</p>{items.length ? <button aria-label={`${items.length} interviews on ${formatCalendarDate(date)}`} className="calendar-month-count mt-3" onClick={() => onSelect(items[0]!)} type="button">{items.length}</button> : null}</div>; })}</div></div>;
}

function TimeGrid({ businessTimeZone, dates, enabledTimezones = [businessTimeZone, secondaryTimezone], grouped, view, now, onSelect, onSelectSlot, onReschedule }: { businessTimeZone: string; dates: Date[]; enabledTimezones?: string[]; grouped: Map<string, CalendarItem[]>; view: View; now: Date; onSelect: (item: CalendarItem) => void; onSelectSlot?: (slot: { date: Date; hour: number }) => void; onReschedule?: (item: CalendarItem, date: Date, hour: number) => Promise<void> }) {
  const dayColumns = view === "day" ? 1 : 7;
  const gridTemplateColumns = `4.5rem repeat(${dayColumns}, minmax(9rem, 1fr))`;
  const currentDateKey = dateKeyInTimezone(now, businessTimeZone);
  const nowParts = timeParts(now.toISOString(), businessTimeZone);
  const currentMinutes = nowParts.hour * 60 + nowParts.minute;
  const currentTop = Math.max(0, Math.min(hours.length * 64 - 2, (currentMinutes / 60) * 64));
  const visibleTimezoneLabels = enabledTimezones.filter((timezone) => timezone !== businessTimeZone);
  const secondaryDelta = timezoneHourDelta(now, businessTimeZone, secondaryTimezone);
  const businessTimeZoneLabel = timezoneOptions.find(([value]) => value === businessTimeZone)?.[1] ?? businessTimeZone;
  return <div className="min-w-[720px]" data-testid="calendar-time-grid"><div className="calendar-time-header sticky top-0 z-20 grid border-b border-border bg-surface-subtle" style={{ gridTemplateColumns, gridTemplateRows: "2rem 2.25rem" }}><div className="calendar-time-rail-header" style={{ gridColumn: "1", gridRow: "1 / span 2" }}><strong>{businessTimeZoneLabel}</strong><span>{visibleTimezoneLabels.includes(secondaryTimezone) ? "PKT" : "Time"}</span><span className="sr-only" data-testid="calendar-timezone-us">US Eastern</span><span className="sr-only" data-testid="calendar-timezone-pakistan">Pakistan</span></div>{dates.map((date) => <p className={`border-l border-border px-2 py-2 text-center text-[10px] font-bold uppercase tracking-[0.12em] ${dateKey(date) === currentDateKey ? "text-primary" : "text-muted-foreground"}`} key={dateKey(date)} style={{ gridColumn: dates.indexOf(date) + 2, gridRow: "1 / span 2" }}>{formatCalendarDate(date)}</p>)}</div><div className="grid" style={{ gridTemplateColumns }}><div className="calendar-time-rail" data-testid={`calendar-timezone-column-${businessTimeZone.replaceAll("/", "-")}`}>{hours.map((hour) => <div className="calendar-time-cell" data-testid="calendar-hour" key={hour}><strong>{formatHour(hour)}</strong>{visibleTimezoneLabels.includes(secondaryTimezone) ? <small>{formatHour((hour + secondaryDelta + 24) % 24)}</small> : null}</div>)}</div>{dates.map((date) => <div className="relative border-l border-border" data-testid="calendar-day-column" key={dateKey(date)} onDragOver={(event) => { if (onReschedule) event.preventDefault(); }} onDrop={(event) => { const id = event.dataTransfer.getData("text/orbit-interview"); const item = Array.from(grouped.values()).flat().find((candidate) => candidate.kind === "interview" && candidate.value.id === id); if (item && onReschedule) { const rect = event.currentTarget.getBoundingClientRect(); void onReschedule(item, date, Math.max(0, Math.min(23, Math.floor((event.clientY - rect.top) / 64)))); } }}><div className="contents">{hours.map((hour) => <button aria-label={`Create interview at ${hour}:00`} className="block h-16 w-full border-b border-border text-left hover:bg-primary-soft/30 disabled:pointer-events-none" data-testid="calendar-hour-slot" disabled={!onSelectSlot} key={hour} onClick={() => onSelectSlot?.({ date, hour })} type="button" />)}</div>{dateKey(date) === currentDateKey ? <div aria-label={`Current time: ${formatClock(now, businessTimeZone)} ${businessTimeZoneLabel}`} className="pointer-events-none absolute inset-x-0 z-10 flex items-center" data-testid="calendar-current-time" style={{ top: currentTop }}><span className="-ml-1.5 size-2.5 rounded-full bg-danger" /><span className="h-px flex-1 bg-danger" /><span className="absolute left-2 top-1 rounded-full bg-danger px-1.5 py-0.5 text-[9px] font-bold text-white">{formatClock(now, businessTimeZone)}</span></div> : null}{(grouped.get(dateKey(date)) ?? []).map((item) => <PositionedEvent businessTimeZone={businessTimeZone} item={item} key={`${item.kind}-${item.value.id}`} onSelect={onSelect} />)}</div>)}</div></div>;
}
...
function PositionedEvent({ businessTimeZone = primaryTimezone, item, onSelect }: { businessTimeZone?: string; item: CalendarItem; onSelect: (item: CalendarItem) => void }) {
  const { startsAt, endsAt } = itemDates(item);
  const start = timeParts(startsAt, businessTimeZone);
  ...
  return <div className="absolute inset-x-1" style={{ top, height: Math.max(38, duration / 60 * 64) }}><EventButton businessTimeZone={businessTimeZone} item={item} onSelect={onSelect} draggable={item.kind === "interview"} /></div>;
}

function EventButton({ businessTimeZone = primaryTimezone, item, onSelect, compact = false, draggable = false }: { businessTimeZone?: string; item: CalendarItem; onSelect: (item: CalendarItem) => void; compact?: boolean; draggable?: boolean }) {
...
function ScheduleInterviewDrawer({ leads, closers, leadTimezones, slot, onClose, onSuccess }: { leads: LeadSummary[]; closers: UserSummary[]; leadTimezones: Record<string, string>; slot: SelectedSlot; onClose: () => void; onSuccess: () => void }) {
  ...
  const [defaultValues, setDefaultValues] = useState(slot.initialValues);
  const [startsAt, setStartsAt] = useState(slot.initialValues?.startsAt ?? "");
  const [endsAt, setEndsAt] = useState(slot.initialValues?.endsAt ?? "");
  const [startsAtEdited, setStartsAtEdited] = useState(false);
  const [endsAtEdited, setEndsAtEdited] = useState(false);
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string>();
  const timezone = leadTimezones[leadId];
  const missingTimezoneError = leadId && !timezone ? "Candidate timezone is unavailable for this application." : undefined;
  const selectLead = (nextLeadId: string) => {
    setLeadId(nextLeadId);
    setError(undefined);
    const nextTimezone = leadTimezones[nextLeadId];
    if (!nextTimezone) {
      setDefaultValues(undefined);
      return;
    }
    try {
      const nextValues = slotDateTimeValues(slot.startsAtIso, nextTimezone);
      setDefaultValues(nextValues);
      setStartsAt(nextValues.startsAt);
      setEndsAt(nextValues.endsAt);
      setStartsAtEdited(false);
      setEndsAtEdited(false);
    } catch (cause) {
      setDefaultValues(undefined);
      setError(cause instanceof Error ? cause.message : "Interview time could not be prepared.");
    }
  };
  const submit = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); setError(undefined); if (!timezone) { setError("Candidate timezone is unavailable for this application."); return; } if (!defaultValues) { setError("Interview time could not be prepared."); return; } setPending(true); try { const startsAtIso = startsAtEdited ? zonedLocalDateTimeToIso(startsAt, timezone) : defaultValues.startsAtIso; const endsAtIso = endsAtEdited ? zonedLocalDateTimeToIso(endsAt, timezone) : defaultValues.endsAtIso; if (endsAtIso <= startsAtIso) throw new Error("End time must be after the start time."); await createLeadInterview(leadId, { closerId, roundType, startsAt: startsAtIso, endsAt: endsAtIso, timezone, originalDatetimeText: `${startsAt}–${endsAt} (${timezone})` }); onSuccess(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Interview could not be scheduled."); setPending(false); } };
  return <div className="fixed inset-0 z-50 flex justify-end bg-sidebar/30" role="presentation" onClick={onClose}><Card aria-label="Schedule interview" className="h-full w-full max-w-lg rounded-l-3xl rounded-r-none overflow-y-auto p-6 shadow-xl" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">New interview</p><h2 className="mt-2 text-xl font-bold text-foreground">Schedule interview</h2><p className="mt-1 text-sm text-muted-foreground">Times use the selected candidate’s timezone.</p></div><button aria-label="Close schedule interview" className="text-2xl leading-none text-muted-foreground" onClick={onClose} type="button">×</button></div><form className="mt-6 grid gap-4" onSubmit={submit}><label className="grid gap-1 text-sm font-semibold text-foreground">Application<select className="h-11 rounded-xl border border-border bg-surface px-3 text-sm font-normal" onChange={(event) => selectLead(event.target.value)} required value={leadId}><option value="">Select an application</option>{leads.map((lead) => <option key={lead.id} value={lead.id}>{lead.jobTitle}{lead.companyName ? ` · ${lead.companyName}` : ""}</option>)}</select></label><label className="grid gap-1 text-sm font-semibold text-foreground">Closer<select className="h-11 rounded-xl border border-border bg-surface px-3 text-sm font-normal" onChange={(event) => setCloserId(event.target.value)} required value={closerId}><option value="">Select a closer</option>{closers.map((closer) => <option key={closer.id} value={closer.id}>{closer.displayName}</option>)}</select></label><label className="grid gap-1 text-sm font-semibold text-foreground">Round type<select className="h-11 rounded-xl border border-border bg-surface px-3 text-sm font-normal" onChange={(event) => setRoundType(event.target.value as CreateInterview["roundType"])} value={roundType}><option value="TECHNICAL">Technical</option><option value="RECRUITER">Recruiter</option><option value="HR">HR</option><option value="FINAL">Final</option><option value="OTHER">Other</option></select></label><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1 text-sm font-semibold text-foreground">Starts<input className="h-11 rounded-xl border border-border bg-surface px-3 text-sm font-normal" onChange={(event) => { setStartsAt(event.target.value); setStartsAtEdited(true); }} required type="datetime-local" value={startsAt} /></label><label className="grid gap-1 text-sm font-semibold text-foreground">Ends<input className="h-11 rounded-xl border border-border bg-surface px-3 text-sm font-normal" onChange={(event) => { setEndsAt(event.target.value); setEndsAtEdited(true); }} required type="datetime-local" value={endsAt} /></label></div><p className="text-xs text-muted-foreground">Timezone: {timezone ?? "Unavailable"}</p><div className="flex items-center gap-3"><Button disabled={pending || !leadId || !closerId || !timezone || !defaultValues} loading={pending} type="submit">{pending ? "Scheduling…" : "Schedule interview"}</Button><Button disabled={pending} onClick={onClose} type="button" variant="secondary">Cancel</Button></div>{error || missingTimezoneError ? <p className="text-sm font-semibold text-danger" role="alert">{error ?? missingTimezoneError}</p> : null}</form></Card></div>;
}
