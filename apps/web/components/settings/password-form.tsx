...
export function PasswordForm({ username }: PasswordFormProps) {
  ...
  return (
    <form className="grid max-w-xl gap-5" onSubmit={handleSubmit}>
      <input aria-hidden="true" autoComplete="username" className="sr-only" name="username" readOnly tabIndex={-1} type="text" value={username} />
      ...
      <Button className="w-fit" disabled={pending} loading={pending} type="submit">{pending ? "Changing password…" : "Change password"}</Button>
