...
export default async function TasksRoute() {
  ...
  return <AppShell actor={actor}><div className="mx-auto grid max-w-[1100px] gap-5"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Work queue</p><h1 className="mt-2 text-2xl font-bold text-foreground sm:text-3xl">Tasks</h1><p className="mt-1.5 text-sm text-muted-foreground">Open follow-ups and preparation work assigned within your workspace.</p></div>{tasks.length === 0 ? <EmptyState description="New follow-ups and preparation work will appear here." title="No open tasks" /> : <section className="grid gap-3">{tasks.map((task) => <Card className="p-5" key={task.id}><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">{task.type.replaceAll("_", " ")}</p><h2 className="mt-2 text-base font-bold text-foreground">{task.title}</h2><p className="mt-1 text-sm text-muted-foreground">Due {formatDue(task.dueAt)} · Profile linked</p></div><span className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${task.priority === "HIGH" ? "bg-danger-soft text-danger" : task.priority === "LOW" ? "bg-surface-subtle text-muted-foreground" : "bg-warning-soft text-warning-foreground"}`}>{task.priority}</span></div>{task.description ? <p className="mt-4 text-sm text-muted-foreground">{task.description}</p> : null}<TaskActions taskId={task.id} version={task.version} /></Card>)}</section>}</div></AppShell>;
}
